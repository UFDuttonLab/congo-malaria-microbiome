...
st_1y <- subset(st_1y, Time == "6W" | Time == "3M" | Time == "6M" | Time == "1Y")
...
taxonomic_levels <- get_available_taxonomic_levels(meco_dataset_1y)
...
cat(sprintf("- Response variable levels: %s\n", paste(levels(meco_dataset_1y$sample_table$BeforeMalaria), collapse = ", ")))
cat(sprintf("- Total features available: %d\n", nrow(meco_dataset_1y$otu_table)))
  ...
  performance_plot <- ggplot(melted_data, aes(x = Method, y = value, fill = Feature_Selection)) +
    ...
    labs(title = "Classifier Performance Across All Taxonomic Levels",
         subtitle = sprintf("Analysis of %d samples with %s outcome classification", 
                            nrow(meco_dataset_1y$sample_table),
                            paste(levels(meco_dataset_1y$sample_table$BeforeMalaria), collapse = " vs ")),
...
save(all_results, results_summary, meco_dataset_1y, successful_results, taxonomic_levels,
